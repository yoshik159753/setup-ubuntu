...
|bash-prompt
|bash のプロンプトに git 統合を追加し PS1 を設定
|-
...
|Edge-beta
|Web Browser. (beta ver)
|-

|FileZilla
|機能豊富なグラフィカル FTP/FTPS/SFTP クライアント
|-

|gcloud CLI
|Google Cloud でホストされるリソースやアプリケーションの管理に使用できる一連のツール
|-

|claude-code
|Anthropic の Claude Code CLI ツール
|-

|claude-desktop
|Anthropic の Claude デスクトップアプリ(ベータ版)
|-
...
|macos-like-dock
|ドックを macOS のように表示する設定
|-

|clamshell-mode
|蓋を閉じてもサスペンドしない設定(クラムシェルモード)
|-

|Fcitx5
a|日本語入力(Mozc)のセットアップ

* Fcitx5 + Mozc インストール
* IM システムを Fcitx5 に切り替え
* Mozc を入力メソッドに追加
* US 配列のセミコロン/コロンを入れ替え
|-
|===
